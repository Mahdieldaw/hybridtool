...
These axes stay decomposed rather than collapsing into a score. Different parts of the system need different signals. Question selection reads total damage — that's where the user's attention does the most work. Structural routing reads cascade fragility — where compounding risk lives. The synthesizer's reconstruction burden shows up in degradation. Keeping the axes separate keeps the policy question visible and auditable, rather than dissolved inside a formula.
