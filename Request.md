...
{
  "path": "C:\\Users\\Mahdi\\Desktop\\hybrid\\src\\geometry\\interpretation\\RESTRUCTURE_INSTRUCTIONS.md",
  "content": "# Geometry Interpretation Layer Restructure
...
The observability row reads `pre?.interRegionSignals` at approximately line 657 and displays an \"Inter-region signals\" table at approximately line 781.

**Action:** Remove the interRegionSignals table. Replace with the new model ordering display (showing each model's irreplaceability score and placement position). The pipeline gate result should also be displayed. These are transparent debugging views of what the geometry layer actually computed.
...
1. **Degenerate gate:** If `isDegenerate(substrate)` → verdict `skip_geometry`. This is already handled by the existing degenerate substrate type, but this gate makes it explicit in the result.
...
2. **Trivial convergence gate:**
   ...
   - Meaning: Every model said roughly the same thing and the geometry reflects it. The mapper still receives all content (nothing is filtered), but the pipeline can skip traversal. Content should still be sent to mapper/synthesizer — this flag indicates traversal is unlikely to be productive.
...
3. **Insufficient structure gate:**
   ...
   - Meaning: The geometry has nothing useful to say. Embeddings may be low quality, or the content is too fragmented for geometric analysis. Don't fake structure. Pass through.

4. **Default:** verdict `proceed` — geometry has structure worth using.

**Notes:**
- For trivial_convergence, confidence should be proportional to how far above thresholds the measurements are. E.g., `largestComponentRatio = 0.95` is higher confidence than `0.86`.
- For insufficient_structure, confidence should be proportional to how severe the fragmentation is.
- For proceed, confidence should reflect how clearly the topology has structure (e.g., based on globalStrongDensity, componentCount, etc.).
- Evidence array should contain human-readable strings like `\"largest_component=95%_of_nodes\"`, `\"model_diversity_in_largest=5/6\"`, etc.

### 2.2 NEW: Model Ordering — `modelOrdering.ts`

**File:** `src/geometry/interpretation/modelOrdering.ts`

**Purpose:** Score each model by geometric irreplaceability — how much unique geometric terrain this model is the sole or primary carrier of. Then arrange models in outside-in order for context window placement (strongest at edges, weakest in middle) to exploit LLM primacy/recency attention bias. This is ordering, not filtering — every model's complete text is preserved in its original paragraph order.
...
**Breakdown fields:**
- `soloCarrierRegions`: count of regions where `modelDiversity == 1` for this model. Pure count.
- `lowDiversityContribution`: sum of `(m's paragraphs in r) / (total paragraphs in r)` only for regions where `modelDiversity <= 2`.
- `totalParagraphsInRegions`: count of this model's paragraphs that are inside any region (vs orphaned).
...
**Edge cases:**
- If `regions.length === 0` (geometry had nothing), fall back to original model order `[1, 2, 3, 4, 5, 6]`.
...
**Future composition note (do not implement now):**

When query relevance is integrated, the final ordering will multiply:
```
finalScore(m) = irreplaceability(m) × mean(queryRelevanceScores for m's paragraphs)
```
This ensures a model that is geometrically unique but off-topic doesn't get prime position. The `computeModelOrdering` function should accept an optional `queryRelevanceBoost?: Map<number, number>` parameter that defaults to `undefined`. When provided, multiply each model's irreplaceability by its boost before sorting. The `ModelScore` type should include a `queryRelevanceBoost?: number` field.
...
export function buildPreSemanticInterpretation(
    ...
    const modelOrdering = computeModelOrdering(
        regionization.regions, regionProfiles, substrate
    );
...
3. **No orphan imports in `ui/`:** Search `ui/` for `interRegionSignal`, `oppositions`, `MapperGeometricHints`. Update `DecisionMapObservabilityRow.tsx` to remove the inter-region signals table.
...
| `ui/components/DecisionMapObservabilityRow.tsx` | UPDATE (remove interRegionSignals table, add new displays) |
...
10. Update `ui/` observability component
...
The write to the user's filesystem failed. Let me try the Desktop Commander approach.
