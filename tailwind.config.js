...
module.exports = {
  darkMode: 'class',
  content: ['./ui/**/*.{js,jsx,ts,tsx}', './src/**/*.{js,jsx,ts,tsx}', './public/**/*.html'],
  theme: {
    extend: {
      fontFamily: {
      ...
      backgroundImage: {
        ...
        'singularity-gradient': 'linear-gradient(135deg, #0A0A0B 0%, #2D3748 50%, #F8F9FA 100%)',

        // App background – Void Black → Convergence Blue → Void Black (horizontal gradient)
        'app-gradient': 'linear-gradient(90deg, #0A0A0B 0%, #2D3748 50%, #0A0A0B 100%)',

        // Processing Gradient: Neural Teal → Insight Amber
        'processing-gradient': 'linear-gradient(90deg, #4A9B8E 0%, #F6AD55 100%)',
