...
describe('SessionManager artifact sanitization', () => {
  it('preserves deep traversalAnalysis arrays when extracting artifacts', () => {
    ...
    const counts = (mapping as any)?.artifact?.traversalAnalysis?.conditionals?.conditions?.[0]
      ?.affectedClaims?.[0]?.stanceAnalysis?.stanceCounts;
    const statements = (mapping as any)?.artifact?.traversalAnalysis?.conditionals?.conditions?.[0]
      ?.affectedStatements;
