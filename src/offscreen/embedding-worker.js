...
async function ensureModel(modelId = 'bge-base-en-v1.5') {
  ...
  inFlightLoad = (async () => {
    try {
      ...
      const shouldTryWebGPUFirst =
        webgpuSupported && hasWebGPU && pipelineOptions.dtype !== 'q4f16';
      ...
      if (shouldTryWebGPUFirst) {
        try {
          ...
          console.log(
            `[EmbeddingWorker] Loaded with WebGPU in ${Math.round(performance.now() - startTime)}ms`
          );
        } catch (webgpuError) {
          ...
          currentBackend = 'wasm';
          console.log(
            `[EmbeddingWorker] Loaded with WASM fallback in ${Math.round(performance.now() - startTime)}ms`
          );
        }
      } else {
        model = await AutoModel.from_pretrained(modelPath, optsForDevice('wasm'));
        currentBackend = 'wasm';
        console.log(
          `[EmbeddingWorker] Loaded with WASM in ${Math.round(performance.now() - startTime)}ms`
        );
      }
...
async function generateEmbeddings(
  ...
  for (let i = 0; i < texts.length; i += batchSize) {
    ...
    const isTyped =
      data &&
      typeof data === 'object' &&
      (data instanceof Float32Array || ArrayBuffer.isView(data));
  ...
  try {
    switch (type) {
      ...
      case 'EMBED': {
        ...
        const result = await generateEmbeddings(texts, dimensions, modelId, {
          yieldBetweenBatches,
          yieldMs,
        });
