...
export const EXCLUSION_RULES: ExclusionRule[] = [
  ...
  // ===================================================================
  {
    ...
    pattern:
      /\b(long\s+before|just\s+before|shortly\s+before|right\s+before|the\s+day\s+before)\b/i,
...
export function isExcluded(
  text: string,
  stance: Stance,
  opts?: { isListItem?: boolean }
): ExclusionResult {
  const applicableRules = EXCLUSION_RULES.filter((r) => r.appliesTo.includes(stance));
