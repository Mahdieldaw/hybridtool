...
import { AUTH_COOKIES, GEMINI_VARIANTS } from './auth-config';
import { getHealthTracker } from './health/provider-health-tracker.js';
...
class AuthManager {
      ...
      AUTH_COOKIES.map(async (config) => {
        try {
          ...
          if (isAuthenticated) {
            ...
            console.log(
              `[AuthManager] ${config.provider}: cookie missing - keeping open, will verify on use`
            );
          }
        } catch (e) {
  ...
  async verifyProvider(providerId) {
    ...
    for (const pid of providerIds) {
      if (this._cookieStatus[pid] !== valid) {
        console.log(
          `[AuthManager] Verification updated status for ${pid}: ${this._cookieStatus[pid]} → ${valid}`
        );
  ...
  async _verifyQwen() {
    ...
    return (
      /csrfToken["']?\s*[:=]\s*["']([^"']+)["']/i.test(html) || html.includes('name="csrf-token"')
    );
  }
  ...
  async handleCookieChange(changeInfo) {
    try {
      ...
      const match = AUTH_COOKIES.find(
        (c) => cookie.domain.includes(c.domain) && cookie.name === c.name
      );
