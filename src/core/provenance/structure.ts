...
const detectChainPattern = (claims: EnrichedClaim[], graph: GraphAnalysis): SecondaryPattern | null => {
...
const detectConditionalPattern = (claims: EnrichedClaim[], edges: Edge[]): SecondaryPattern | null => {
...
export function analyzeGlobalStructure(input: {
  ...
  while (currentClaims.length >= 2) {
    ...
    const currentSalient = currentClaims
      .filter((c) => c.supporters.length > meanK)
      .sort((a, b) => b.supporters.length - a.supporters.length);
    const layer = detectShape(currentSalient, currentClaims, currentTotalMass, safeEdges, modelCount);
