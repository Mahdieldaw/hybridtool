...
function computeCrossPoolProximityStatements(
  ...
  if (embsExclA.length === 0 || embsExclB.length === 0 || allEmbsA.length === 0 || allEmbsB.length === 0)
    return null;

  let sumAtoB = 0;
  for (const ea of embsExclA) {
    let maxSim = -Infinity;
    for (const eb of allEmbsB) { const s = cosineSimilarity(ea, eb); if (s > maxSim) maxSim = s; }
    sumAtoB += maxSim;
  }
  ...
  for (const eb of embsExclB) {
    let maxSim = -Infinity;
    for (const ea of allEmbsA) { const s = cosineSimilarity(eb, ea); if (s > maxSim) maxSim = s; }
    sumBtoA += maxSim;
  }
  ...
  for (const [claimId, profile] of Object.entries(densityResult.profiles)) {
    ...
    for (const passage of profile.passages) {
      for (let pi = passage.startParagraphIndex; pi <= passage.endParagraphIndex; pi++) {
        ...
        if (!arr) { arr = []; membership.set(key, arr); }
...
function buildParaCoverage(densityResult: ClaimDensityResult): Map<string, ParaCoverageItem[]> {
  ...
  for (const [claimId, profile] of Object.entries(densityResult.profiles)) {
    for (const pc of profile.paragraphCoverage) {
      ...
      if (!arr) { arr = []; map.set(pc.paragraphId, arr); }
...
function computeAllegiance(
  ...
  if (para && stmtEmb) {
    ...
    const calibrationWeight = para.statementIds.length > 0 ? calibrationPool.length / para.statementIds.length : 0;
    ...
    if (calibrationPool.length >= 2) {
      ...
      if (dominantCentroid) {
        ...
        for (const rivalId of rivalClaimIds) {
          ...
          rivalAllegiances.push({ claimId: rivalId, rawAllegiance, weightedAllegiance: rawAllegiance * calibrationWeight });
        }
        ...
        if (rivalAllegiances.length > 0) {
          ...
          for (const ra of rivalAllegiances) {
            ...
            if (abs > maxAbs) { maxAbs = abs; bestRival = ra; }
          }
          ...
          if (value !== null && value > 0) {
            ...
            for (const ra of rivalAllegiances) {
              if (ra.weightedAllegiance < strongestPull) { strongestPull = ra.weightedAllegiance; strongestRival = ra.claimId; }
            }
          ...
          return {
            allegiance: { value, calibrationWeight, dominantClaimId, rivalAllegiances, method: 'calibrated' },
            primaryClaim,
          };
        }
      }
    ...
    if (centroidSims.length > 0) {
      ...
      for (let i = 1; i < centroidSims.length; i++) {
        ...
        rivalAllegiances.push({ claimId: centroidSims[i].claimId, rawAllegiance: diff, weightedAllegiance: diff });
      }
      return {
        allegiance: {
  ...
  if (para) {
    ...
    if (passages && passages.length > 0) {
      ...
      if (bestOwner) {
        return {
          allegiance: { value: null, calibrationWeight: 0, dominantClaimId, rivalAllegiances: [], method: 'passage-dominance' },
  ...
  return {
    allegiance: { value: null, calibrationWeight: 0, dominantClaimId, rivalAllegiances: [], method: null },
    primaryClaim: null,
  };
}
...
function computePassageDominance(
  ...
  return { inPassage: true, passageOwner: best.claimId, coverageFraction: best.coverageFraction, passageLength: best.passageLength };
}
...
export function validateEdgesAndAllegiance(input: ValidateInput): ValidateOutput {
  ...
  for (let i = 0; i < claims.length; i++) {
    for (let j = i + 1; j < claims.length; j++) {
      ...
      const triangleResult = claimEmbeddings && queryEmb
  ...
  let resolvedByCalibration = 0, resolvedByCentroidFallback = 0, resolvedByPassageDominance = 0, unresolved = 0;
  ...
  for (const [stmtId, owners] of ownershipMap) {
    ...
    const { allegiance, primaryClaim } = computeAllegiance(
      stmtId, stmtEmb, paragraphId ?? '', assignedClaims,
      paraById, paraCoverage, ownershipMap,
      stmtEmbeddingsNonNull, claimEmbeddings, passageMembership
    );
    ...
    entries[stmtId] = {
      ...
      assignedClaims,
      primaryClaim,
      secondaryClaims: primaryClaim ? assignedClaims.filter((id) => id !== primaryClaim) : [...assignedClaims],
      allegiance,
  ...
  const totalJoint = resolvedByCalibration + resolvedByCentroidFallback + resolvedByPassageDominance + unresolved;

  return {
    validatedConflicts,
    provenanceRefinement: {
      entries,
      summary: { totalJoint, resolvedByCalibration, resolvedByCentroidFallback, resolvedByPassageDominance, unresolved },
