...
export async function measureProvenance(input: MeasurePhaseInput): Promise<MeasurePhaseOutput> {
  ...
  if (claims.length > 1 && Array.from(claimPools.values()).every((p) => p.length === 0)) {
    ...
    console.log('[Provenance] Degenerate guard fired: assigned all', simMatrix.size, 'paragraphs to all', claims.length, 'claims');
  }

  console.log(`[Provenance] Competitive assignment: ${claims.length} claims, ${simMatrix.size} paragraphs, ${claimEmbeddings.size} centroids`);
  console.log(`[Provenance] Pool sizes: ${claims.map((c) => `${c.id}:${claimPools.get(c.id)?.length ?? 0}`).join(', ')}`);
  ...
  let totalKept = 0, totalInCompetitive = 0, totalExpanded = 0, totalRemoved = 0, totalMergedStmts = 0;

  for (const claim of claims) {
    const claimEmb = claimEmbeddings.get(claim.id);

    // Claim-centric pool (μ+σ per claim over all paragraphs)
    let ccMu = 0, ccSigma = 0, ccThreshold = 0;
    ...
    let bothCount = 0, compOnlyCount = 0, ccOnlyCount = 0;
    for (const paraId of allParaIds) {
    ...
    for (const sid of canonicalStatementIds) {
      ...
      if (!owners) { owners = new Set(); ownershipMap.set(sid, owners); }
      owners.add(claim.id);
    }
    ...
    for (const pc of paragraphCoverage) {
      ...
      if (!arr) { arr = []; byModel.set(pc.modelIndex, arr); }
      arr.push(pc);
    }
    ...
    let maxPassageLength = 0, meanCoverageInLongestRun = 0;
    ...
    densityProfiles[claim.id] = {
      ...
      modelsWithPassages: new Set(passages.filter((p) => p.length >= 2).map((p) => p.modelIndex)).size,
    ...
    for (const paraId of allParaIds) provenanceBulk += normalizedWeights.get(paraId)?.get(claim.id) ?? 0;
  ...
  console.log(`[Provenance] Canonical statement counts: ${enrichedClaims.map((r) => `${r.id}:stmts=${r.sourceStatementIds?.length ?? 0}`).join(', ')}`);
