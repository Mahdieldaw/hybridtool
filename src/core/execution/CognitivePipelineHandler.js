      ...
      if (stepExecutor && streamingManager && !singularityDisabled) {
        ...
        try {
          ...
          if (needsFreshInstance) {
            console.log(
              `[CognitiveHandler] Fresh instance needed: first=${!conciergeState?.hasRunConcierge}, providerChanged=${providerChanged}, commitPending=${handoffV2Enabled && conciergeState?.commitPending}`
            );
          }
          ...
          if (conciergeState?.lastProcessedTurnId === context.canonicalAiTurnId) {
            console.warn(
              `[CognitivePipeline] Turn ${context.canonicalAiTurnId} already processed (idempotency safety net).`
            );
            return true;
          }
            ...
            if (!handoffV2Enabled || turnInCurrentInstance === 1) {
              ...
              try {
                ...
                if (evidenceSubstrate) {
                  console.log(
                    `[CognitiveHandler] Built evidence substrate: ${evidenceSubstrate.length} chars`
                  );
                }
              } catch (substrateErr) {
  ...
  async handleContinueRequest(payload, stepExecutor, streamingManager, contextManager) {
    ...
    try {
      ...
      try {
          ...
          try {
            ...
            if (
              ...
              try {
                ...
                console.warn(
                  '[CognitiveHandler] Shadow extraction failed:',
              ...
              console.log(
                `[CognitiveHandler] Rebuilt artifact via buildArtifactForProvider: ${buildResult.enrichedClaims.length} claims`
              );
            }
          } catch (rebuildErr) {
            console.warn(
              '[CognitiveHandler] buildArtifactForProvider failed (fallback to parse):',
              rebuildErr
            );
          }
        }
          ...
          if (editorialResponse?.text) {
            try {
              ...
              if (parsed.success && parsed.ast) {
                ...
                console.log(
                  `[CognitiveHandler] Restored editorialAST: ${parsed.ast.threads.length} thread(s)`
                );
              }
            } catch (editorialErr) {
              console.warn(
                '[CognitiveHandler] Editorial AST restoration failed (non-blocking):',
                editorialErr
              );
            }
          }
        ...
        try {
          ...
          if (evidenceSubstrate) {
            ...
            if (ConciergeService && typeof ConciergeService.buildConciergePrompt === 'function') {
              ...
              console.log(
                `[CognitiveHandler] Built concierge prompt with evidence substrate (${evidenceSubstrate.length} chars)`
              );
            }
          }
        } catch (substrateErr) {
          console.warn(
            '[CognitiveHandler] Evidence substrate build failed in continuation:',
            substrateErr
          );
        }
        ...
        const result = await stepExecutor.executeSingularityStep(
          step,
          context,
          new Map(),
          executorOptions
