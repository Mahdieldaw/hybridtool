...
export async function executeMappingPhase(step, context, stepResults, workflowContexts, options) {
  ...
  const rawSourceData = await options.contextManager.resolveHistoricalSources(payload, context, stepResults);
  ...
  const shadowModule = await import('../../../shadow/index.js');
  ...
  console.log(`[executeMappingPhase] Extracting shadow statements from ${shadowInput.length} models...`);
  const shadowResult = extractShadowStatements(shadowInput);
  console.log(`[executeMappingPhase] Extracted ${shadowResult.statements.length} shadow statements.`);
          ...
          onAllComplete: async (results, errors) => {
            ...
            if (finalResult?.text) {
              ...
              if (parseResult.success && parseResult.output) {
                ...
                console.warn('[executeMappingPhase] Semantic Mapper parsing failed:', parseResult.errors);
              }
