...
export function computeBlastSurface(input: BlastSurfaceInput): BlastSurfaceResult {
  ...
  const { claims, statementEmbeddings, totalCorpusStatements, statementTexts } = input;
  ...
  for (const claim of claims) {
    ...
    const cascadeFragilityMu =
      fragValues.length > 0 ? fragValues.reduce((a, b) => a + b, 0) / fragValues.length : 0;
