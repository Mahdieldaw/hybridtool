...
export async function runPreflight(request, authStatus, availableProviders) {
  ...
  if (singularity && authStatus[singularity.toLowerCase()] === false) {
    ...
    if (locks.singularity) {
      if (candidate) {
        warnings.push(
          `Singularity provider "${singularity}" is locked but unauthorized; using "${candidate}" for this request`
        );
        singularity = candidate;
      } else {
        warnings.push(
          `Singularity provider "${singularity}" is locked but unauthorized and no fallback available`
        );
        singularity = 'gemini';
      }
    } else {
