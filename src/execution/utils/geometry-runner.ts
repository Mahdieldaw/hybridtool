...
export async function buildGeometryAsync(
  ...
  try {
    if (
      !paragraphResult ||
      !Array.isArray(paragraphResult.paragraphs) ||
      paragraphResult.paragraphs.length === 0
    ) {
    ...
    if (!results.geometryParagraphEmbeddings || results.geometryParagraphEmbeddings.size === 0) {
      console.log(
        `[buildGeometryAsync] Skipping embeddings/geometry (paragraph_embeddings_unavailable)`
      );
      geometryDiagnostics.stages.embeddingsSkipped = {
        startedAtMs,
        timeMs: 0,
    ...
    try {
      ...
      console.warn(
        '[buildGeometryAsync] isDegenerate check failed, treating substrate as degenerate:',
        err
      );
      results.substrateDegenerate = true;
      results.substrateDegenerateReason = `degeneracy-check-error: ${err instanceof Error ? err.message : String(err)}`;
    }
