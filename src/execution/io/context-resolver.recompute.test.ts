...
import type { ProviderResponseRecord } from '../../../persistence/index.js';
...
describe('ContextResolver recompute(batch)', () => {
  ...
  it('prefers stored turn providerContexts :batch meta when present', async () => {
    const getResponsesByTurnId = jest.fn<Promise<ProviderResponseRecord[]>, [string]>(
      async () => [] as ProviderResponseRecord[]
    );
    const adapter: AdapterStub = {
      isReady: () => true,
      get: async (_store: string, id: string) => {
