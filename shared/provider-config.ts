 ...
 * survey mapper, synthesis, UI citations, and REGENERATE paths.
 *
 ...
 */
export function canonicalCitationOrder(
    activeProviderIds: string[],
): string[] {
    const normalized = activeProviderIds.map(p => String(p || '').trim().toLowerCase()).filter(Boolean);
 ...
 */
export function buildCitationSourceOrder(
