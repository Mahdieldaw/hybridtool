...
export function parseSemanticMapperOutput(rawResponse: string): SemanticMapperParseResult {
  ...
  let narrative =
    narrativeMatches.length > 0
      ? String(narrativeMatches[narrativeMatches.length - 1]?.[1] || '').trim()
      : '';
  ...
  if (!parsedMap || typeof parsedMap !== 'object') {
    return {
      success: false,
      errors: [{ field: 'response', issue: 'No valid JSON found in response' }],
    };
  }
  ...
  if (rawClaims.length === 0) {
    return {
      success: false,
      errors: [{ field: 'claims', issue: 'Missing or empty claims array' }],
    };
  }
