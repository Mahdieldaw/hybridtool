...
function buildValleyCandidates(
  peaksByIndex: PeakCandidate[],
  xs: number[],
  ys: number[],
  dx: number | null
  ...
  for (let i = 0; i < peaksByIndex.length - 1; i++) {
    ...
    const curvature =
      valleyIdx > 0 && valleyIdx < ys.length - 1 && dx != null && dx > 0 && Number.isFinite(dx)
...
function deriveAdaptiveLadder(
  ...
  const regimeProxy =
    discriminationRange != null && scale != null && scale > 0 ? discriminationRange / scale : 0.5;
...
export function computeBasinInversion(
  ...
  const selectValleyFromEntry = (entry: SweepEntry | null): SelectedValley | null => {
    ...
    const candidates = entry.valleys.filter(
      (v) => v.promMin >= alpha && v.curvature != null && v.curvature > curvatureThreshold
    );
  ...
  const displayEntry =
    ...
    (hBase != null
      ? sweep.reduce<SweepEntry | null>((best, entry) => {
          ...
          return Math.abs(entry.bandwidth - hBase) < Math.abs(best.bandwidth - hBase)
            ? entry
            : best;
        }, null)
      : null);
  ...
  const peaks: BasinInversionPeak[] = displayEntry
    ? peakCandidates.map((p) => {
  ...
  if (displayBandwidth != null && displayBandwidth > 0 && binWidth > 0) {
    ...
    binnedPeakCenters = binnedSelected
      ? [binnedSelected.peakA.center, binnedSelected.peakB.center]
      : null;
    ...
    if ((selected == null) !== (binnedSelected == null)) {
      ...
      const binnedBins = [binnedSelected.peakA.index, binnedSelected.peakB.index].sort(
        (a, b) => a - b
      );
    ...
    meta: {
      ...
      peakDetection: {
        ...
        selectedPeaks: selected
          ? [
              {
                center: selected.peakA.center,
                height: selected.peakA.height,
                prominenceSigma: selected.promA,
              },
              {
                center: selected.peakB.center,
                height: selected.peakB.height,
                prominenceSigma: selected.promB,
              },
            ]
          : [],
        valley: selected
