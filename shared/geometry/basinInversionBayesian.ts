...
function computeNodeProfile(
    ...
    if (N < 3) {
        return {
            nodeIndex, sortedSims, sortedPeerIndices,
            changePoint: null, boundarySim: null,
...
export function computeBasinInversionBayesian(
    ...
    if (nodeCount < 2) {
        return {
            status: "insufficient_data", statusLabel: "Insufficient Data",
            nodeCount, pairCount: 0,
            mu: null, sigma: null, p10: null, p90: null, discriminationRange: null,
            binCount: 0, binMin: 0, binMax: 1, binWidth: 1,
            histogram: [], histogramSmoothed: [], peaks: [],
            T_low: null, T_high: null, T_v: null,
            pctHigh: null, pctLow: null, pctMid: null, pctValleyZone: null,
            basinCount: 1, largestBasinRatio: nodeCount > 0 ? 1 : null,
    ...
    for (let i = 0; i < nodeCount; i++) {
        ...
        if (bid == null) { bid = nextBasin++; rootToBasin.set(r, bid); }
        nodeBasin[i] = bid;
    }
    ...
    for (let i = 0; i < nodeCount; i++) {
        ...
        if (arr) arr.push(ids[i]); else basinMembers.set(bid, [ids[i]]);
    }
    const basinsSorted = Array.from(basinMembers.entries())
    ...
    if (basinCount <= 1) {
        ...
        statusLabel = nodesWithBoundary === 0
    ...
    if (T_v != null) {
        for (let p = 0; p < pairCount; p++) {
            ...
            if (Math.abs(s - T_v) <= halfBinWidth) {
                valleyCount++;
                bridgePairs.push({
                    nodeA: ids[pairI[p]], nodeB: ids[pairJ[p]], similarity: s,
                    basinA: nodeBasin[pairI[p]], basinB: nodeBasin[pairJ[p]],
    ...
    return {
        status, statusLabel, nodeCount, pairCount,
        mu, sigma, p10, p90, discriminationRange,
        binCount, binMin, binMax, binWidth,
        histogram, histogramSmoothed: histogram.slice(),
        peaks: [] as BasinInversionPeak[],
        T_low, T_high, T_v,
        pctHigh, pctLow, pctMid, pctValleyZone,
        basinCount, largestBasinRatio, basinByNodeId,
        basins, bridgePairs,
        meta: {
            ...
            peakDetection: {
                bandwidth: null, bandwidthSigma: sigma, bandwidthN: pairCount,
