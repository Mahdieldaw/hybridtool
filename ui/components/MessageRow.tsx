...
function MessageRow({ turnId }: { turnId: string }) {
  const turnAtom = useMemo(() => selectAtom(turnsMapAtom, (map) => map.get(turnId)), [turnId]);
  const message = useAtomValue(turnAtom) as TurnMessage | undefined;
  const isActiveTurn = useAtomValue(
    useMemo(() => selectAtom(activeSplitPanelAtom, (p) => p?.turnId === turnId), [turnId])
  );

  if (!message) {
    return <div className="p-2 text-intent-danger">Error: Missing turn {turnId}</div>;
  }
