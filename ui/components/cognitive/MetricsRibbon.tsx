...
const Agree: React.FC<{ agree: boolean }> = ({ agree }) => (
    <span className={agree ? "text-emerald-400/70" : "text-amber-400/70"}>
        {agree ? "✓" : "⚠"}
    </span>
);
...
function computeDualSignals(
    ...
    if (passageRouting) {
        ...
        passageBacked = profiles.filter(p => p.landscapePosition === 'northStar' || p.landscapePosition === 'mechanism').length;
        passageWeak = profiles.filter(p => p.landscapePosition === 'eastStar' || p.landscapePosition === 'floor').length;
    }
    ...
    return {
        structConflicts, geoConflicts, geoOnly, mapperOnly, conflictsAgree,
        highSupportPct, highSupportCount, passageBacked, passageWeak,
        hubId, hubLabel, hubZ, geoHubId, geoHubLabel, geoHubConcentration, hubAgree,
        componentCount, regionCount,
    };
}
                ...
                <div className="mt-4 pt-3 border-t border-border-subtle text-xs text-text-muted space-y-2">
                    ...
                    {hasAnyGeo && (
                        <div className="mt-4 pt-3 border-t border-border-subtle space-y-3 min-w-0 overflow-hidden">
                            <div className="text-[10px] text-text-muted/50 uppercase tracking-widest">Mapper vs Geometry</div>
                            ...
                            <DualRow label="Conflicts" agree={dual.conflictsAgree}>
                                ...
                                {dual.mapperOnly != null && dual.mapperOnly > 0 && (
                                    <span className="text-[10px] text-amber-400/70">{dual.mapperOnly} mapper-only</span>
                                )}
                            </DualRow>
                            ...
                            <div className="min-w-0">
                                ...
                                <div className="space-y-1.5 text-xs min-w-0">
                                    <div className="flex items-center gap-2 min-w-0">
                                        ...
                                        {dual.hubLabel ? (
                                            <>
                                                <span className="text-text-primary truncate min-w-0" title={dual.hubLabel}>{dual.hubLabel}</span>
                                                <span className="text-text-muted/40 shrink-0">z={dual.hubZ?.toFixed(1)}</span>
                                            </>
                                        ) : (
                                    ...
                                    <div className="flex items-center gap-2 min-w-0">
                                        ...
                                        {dual.geoHubId !== null ? (
                                            <>
                                                <span className="text-text-primary truncate min-w-0" title={dual.geoHubLabel ?? undefined}>{dual.geoHubLabel}</span>
                                                {dual.geoHubConcentration !== null && (
                                                    <span className="text-text-muted/40 shrink-0">c={dual.geoHubConcentration.toFixed(2)}</span>
                                                )}
                                                {dual.hubAgree !== null && <span className="shrink-0"><Agree agree={dual.hubAgree} /></span>}
                                            </>
                                        ) : (
