...
export function useSingularityMode(trackedAiTurnId?: string) {
  ...
  const isTransitioning =
    !!trackedAiTurnId && globalIsLoading && activeAiTurnId === trackedAiTurnId;

  const runSingularity = useCallback(
    async (aiTurnId: string, options: SingularityTransitionOptions = {}) => {
      if (!sessionId) {
      ...
      try {
        ...
        try {
          ...
          console.warn(
            '[useSingularityMode] ensurePort failed prior to transition; proceeding with sendPortMessage',
            e
          );
        }
    ...
    [sessionId, setActiveAiTurnId, setActiveRecomputeState, setGlobalIsLoading, setUiPhase]
  );
