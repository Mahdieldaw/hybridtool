 ...
 */
export function useProviderActions(sessionId: string | undefined, aiTurnId: string) {
  ...
  const handleRetryProvider = useCallback(
    async (
      providerId: string,
      stepType: 'batch' | 'mapping' | 'understand' | 'gauntlet' = 'batch'
    ) => {
      if (!sessionId || !aiTurn) {
  ...
  const handleBranchContinue = useCallback(
