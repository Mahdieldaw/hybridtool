  ...
  const runMappingForAiTurn = useCallback(
    async (aiTurnId: string, providerIdOverride?: string) => {
      ...
      const outputsFromBatch = ai.batch?.responses
        ? Object.values(ai.batch.responses).filter(
      ...
      const enoughOutputs = outputsFromBatch.length >= 2 || hasCompletedMapping;
      if (!enoughOutputs) {
        console.warn(`[RoundActions] Not enough outputs for mapping in turn ${aiTurnId}`);
        setAlertText(
          'Not enough source data to run mapping. Please wait for more providers to finish.'
        );
        return;
      }
      ...
      try {
        ...
        const primitive: PrimitiveWorkflowRequest = {
          ...
          useThinking:
            effectiveMappingProvider === 'chatgpt' ? !!thinkMappingByRound[userTurnId] : false,
        };
