...
export function BayesianBasinCard({
  ...
  const { basins, summary } = useMemo(() => {
    ...
    if (!basinInversion || b.length === 0) return { basins: b, summary: null };
    let nUnchecked = 0;
    let nFound = 0;
    for (const basin of b) {
      if (basin.type === 'unchecked') nUnchecked++;
      else nFound++;
    }
    const nodesInBasins = b.reduce((sum: number, basin: any) => sum + (basin.nodeIds?.length ?? 0), 0);
  ...
  return (
    <div className="space-y-4">
      ...
      <InterpretiveCallout
        text={
          status === 'ok'
            ? `Bayesian inversion identified ${basins.length} stable basin${basins.length !== 1 ? 's' : ''} at Tv=${fmt(basinInversion.T_v, 4)}, covering ${summary != null ? fmtPct(summary.coverage) : '0%'} of paragraphs. Boundaries are distinct (valley density < floor).`
