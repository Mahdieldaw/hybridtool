...
import { formatSingularityResponse } from '../utils/copy-format-utils';
...
const SingularityOutputView: React.FC<SingularityOutputViewProps> = ({
  ...
  const providerHasStoredResponse = (pid: string) => {
    ...
    if (
      normalizeProviderId(directProvider) === desired &&
  ...
  const renderSwitcher = (variant: 'pill' | 'rect' = 'rect') => {
    ...
    return (
      <div className="relative">
        <button onClick={() => setShowRegenMenu(!showRegenMenu)} className={buttonClasses}>
          <span
            className={`w-1.5 h-1.5 rounded-full ${variant === 'pill' ? 'bg-brand-500' : 'bg-emerald-500'}`}
          />
      ...
      return (
        <div className="py-8 space-y-6">
          <PipelineErrorBanner
            ...
            retryable={retryable}
          />
          <div className="flex justify-center">{renderSwitcher('pill')}</div>
        </div>
      );
    }
    ...
    if (effectiveIsLoading && !hasRenderableText) {
      return (
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <div className="text-text-secondary font-medium mt-6">Synthesizing response...</div>
    ...
    return (
      <div className="bg-surface border border-border-subtle rounded-2xl overflow-hidden shadow-sm relative">
        ...
        <div className="relative z-10 px-6 py-4 border-t border-border-subtle/50 bg-surface-highlight/5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <CopyButton text={formatSingularityResponse(output)} label="Copy response" variant="icon" />
            {copyAllText && (
              <CopyButton
                text={copyAllText}
            ...
            <button
              onClick={() => {
                const pid =
                  output?.providerId ??
                  singularityState.requestedProviderId ??
                  singularityState.providerId;
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {output?.leakageDetected &&
        output.leakageViolations &&
        output.leakageViolations.length > 0 && (
