...
export const PipelineErrorBanner: React.FC<PipelineErrorBannerProps> = ({
  ...
  const getDescription = () => {
    ...
    switch (type) {
      case 'batch':
        return 'This model did not return a response for this turn.';
      case 'mapping':
        return 'Advanced insights require a successful cross-reference of multiple sources.';
      case 'singularity':
        return 'The intelligence is currently unavailable.';
      default:
        return 'An error occurred during this pipeline step.';
    }
  };
  ...
  const failedModelName = failedProviderIdSafe
  ...
  return (
    <div
      className={clsx(
        ...
        compact
          ? 'bg-surface-raised/50 border-border-subtle/50'
          : 'bg-intent-warning/5 border-intent-warning/20 shadow-sm'
      )}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {!compact && (
            <p className="text-xs text-text-secondary mt-1 leading-relaxed">{getDescription()}</p>
          )}
          {compact && errorMessage && (
            <p className="text-[11px] text-intent-danger mt-0.5 font-medium">{errorMessage}</p>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mt-1">
        ...
        <div className="relative">
          ...
          >
            Try different model{' '}
            <span className={clsx('transition-transform', showDropdown && 'rotate-180')}>▾</span>
          </button>
          ...
          {showDropdown && (
            <>
              ...
              <div className="absolute bottom-full left-0 mb-2 w-48 bg-surface-raised border border-border-subtle rounded-xl shadow-elevated p-1.5 z-50 animate-in fade-in zoom-in-95 duration-150">
                <div className="text-[10px] text-text-muted px-2 py-1 font-medium uppercase tracking-wider border-b border-border-subtle/30 mb-1">
                  Select Model
                </div>
                <div className="max-h-48 overflow-y-auto no-scrollbar">
                  {LLM_PROVIDERS_CONFIG.filter(
                    (p) => !failedProviderIdSafe || p.id !== failedProviderIdSafe
                  ).map((p) => (
                    <button
                      key={p.id}
                      onClick={() => {
