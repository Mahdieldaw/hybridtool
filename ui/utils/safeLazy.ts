    ...
    const load = async (): Promise<{ default: T }> => {
      try {
        ...
        if ((isNetworkError || isChunkError) && retries < MAX_RETRIES) {
          ...
          console.warn(
            `[safeLazy] Load failed, retrying in ${delay}ms (attempt ${retries}/${MAX_RETRIES})...`,
            error
          );
        ...
        if (reloadCount < MAX_RELOADS) {
          ...
          const ReloadingFallback = () =>
            React.createElement('div', { className: 'loading' }, 'Reloading...');
          ...
          React.createElement(
            ...
            React.createElement(
              'button',
              {
                type: 'button',
                className:
                  'px-4 py-2 bg-surface-raised hover:bg-surface-highlight rounded border border-border-subtle transition-colors',
                onClick: () => {
                  try {
                    sessionStorage.removeItem(RELOAD_COUNTER_KEY);
                  } catch (e) {}
